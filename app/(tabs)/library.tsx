...
// ExercisesView Component with full CRUD
const ExercisesView = () => {
    ...
    const handleAdd = () => { if (!exerciseName.trim()) { Alert.alert('خطأ','يرجى إدخال اسم التمرين'); return; } addExercise({ name: exerciseName.trim(), muscle: selectedMuscle, sessions: parseInt(sessions) || 3, restTime: parseInt(restTime) || 90 }); setShowAddModal(false); };
    const handleEdit = (exercise: Exercise) => { setEditingExercise(exercise); setExerciseName(exercise.name); setSelectedMuscle(exercise.muscle); setSessions(String(exercise.sessions)); setRestTime(String(exercise.restTime)); setShowEditModal(true); };
    const handleUpdate = () => { if (!editingExercise || !exerciseName.trim()) { Alert.alert('خطأ', 'يرجى إدخال اسم التمرين'); return; } updateExercise(editingExercise.id, { name: exerciseName.trim(), muscle: selectedMuscle, sessions: parseInt(sessions) || editingExercise.sessions, restTime: parseInt(restTime) || editingExercise.restTime }); setShowEditModal(false); setEditingExercise(null); };
    ...
    return (
        <View style={styles.viewContainer}>
            ...
            <ScrollView style={styles.listScrollView} showsVerticalScrollIndicator={false}>{exercises.length === 0 ? <View style={styles.emptyContainer}><Text style={styles.emptyText}>لا توجد تمارين مضافة بعد</Text></View> : exercises.map((exercise) => (<TouchableOpacity key={exercise.id} style={[styles.exerciseItem, selectedExercises.has(exercise.id) && styles.selectedExerciseItem, { borderLeftColor: getStatusColor(exercise.status) }]} onPress={() => toggleExerciseSelection(exercise.id)}><View style={styles.exerciseContent}><View style={styles.exerciseInfo}><Text style={styles.exerciseName}>{exercise.name}</Text><Text style={styles.exerciseDetails}>{exercise.muscle} - {exercise.sessions} جلسات</Text><Text style={[styles.exerciseStatus, { color: getStatusColor(exercise.status) }]}>{getStatusText(exercise.status)}</Text></View><View style={styles.exerciseActions}><TouchableOpacity style={[styles.actionButton, { backgroundColor: settings.primaryColor }]} onPress={(e) => { e.stopPropagation(); handleEdit(exercise); }}><Edit size={16} color="white" /></TouchableOpacity><TouchableOpacity style={[styles.actionButton, styles.deleteButton]} onPress={(e) => { e.stopPropagation(); handleDeleteExercise(exercise.id); }}><Trash2 size={16} color="white" /></TouchableOpacity></View></View>{selectedExercises.has(exercise.id) && <View style={styles.selectionIndicator} />}</TouchableOpacity>))}</ScrollView>
            <TouchableOpacity style={[styles.addButton, { backgroundColor: settings.primaryColor }]} onPress={() => { resetForm(); setShowAddModal(true); }}><Plus size={20} color="white" /><Text style={styles.addButtonText}>إضافة تمرين جديد</Text></TouchableOpacity>
            <Modal visible={showAddModal} transparent={true} animationType="slide" onRequestClose={() => setShowAddModal(false)}><View style={styles.modalOverlay}><View style={styles.modalContent}><View style={styles.modalHeader}><Text style={styles.modalTitle}>إضافة تمرين جديد</Text><TouchableOpacity onPress={() => setShowAddModal(false)}><X size={24} color="#6b7280" /></TouchableOpacity></View>{renderExerciseForm(false)}</View></View></Modal>
            <Modal visible={showEditModal} transparent={true} animationType="slide" onRequestClose={() => setShowEditModal(false)}><View style={styles.modalOverlay}><View style={styles.modalContent}><View style={styles.modalHeader}><Text style={styles.modalTitle}>تعديل التمرين</Text><TouchableOpacity onPress={() => setShowEditModal(false)}><X size={24} color="#6b7280" /></TouchableOpacity></View>{renderExerciseForm(true)}</View></View></Modal>
...
const AnalyticsView = () => {
    ...
    const sessionsForSelectedDay = useMemo(() => {
        return sessionHistory
            .filter(session => new Date(session.startTime).toISOString().startsWith(selectedDate))
        ...
        <View style={styles.viewContainer}>
            <View style={styles.analyticsCard}>
                ...
                <Calendar
                    ...
                    theme={{
                        backgroundColor: '#ffffff', calendarBackground: '#ffffff', textSectionTitleColor: '#b6c1cd',
                        selectedDayBackgroundColor: settings.primaryColor, selectedDayTextColor: '#ffffff',
                        todayTextColor: settings.primaryColor, dayTextColor: '#2d4150',
                        arrowColor: settings.primaryColor, monthTextColor: settings.primaryColor,
                    }}
                    style={styles.calendar}
                />
            </View>
            <View style={styles.analyticsCard}>
                <Text style={[styles.cardTitle, { color: settings.primaryColor }]}>جلسات يوم {selectedDate}</Text>
                {sessionsForSelectedDay.length > 0 ? (
                    ...
                    <Text style={styles.emptyText}>لا توجد جلسات في هذا اليوم.</Text>
                )}
            </View>
        </View>
    );
...
const PlansView = () => {
    ...
    const handleActivatePlan = (planId: number) => {
        ...
        if (Platform.OS === 'web') { if (confirm(message)) action(); }
        else { Alert.alert(title, message, [{ text: 'إلغاء', style: 'cancel' }, { text: 'نعم', onPress: action }]); }
    };

    const handleDeletePlan = (planId: number) => {
        if (Platform.OS === 'web') { if (confirm('هل أنت متأكد من حذف خطة التدريب هذه؟')) deleteTrainingPlan(planId); }
        else { Alert.alert('حذف الخطة', 'هل أنت متأكد من حذف خطة التدريب هذه؟', [{ text: 'إلغاء', style: 'cancel' }, { text: 'حذف', style: 'destructive', onPress: () => deleteTrainingPlan(planId) }]); }
    };

    const handleEditPlan = (plan: typeof trainingPlans[0]) => { setEditingPlanId(plan.id); setPlanName(plan.name); setPlanDescription(plan.description); setSelectedExercises(plan.exercises); setShowEditModal(true); };
    const handleAddNewPlan = () => { setPlanName(''); setPlanDescription(''); setSelectedExercises([]); setShowAddModal(true); };

    const handleSavePlan = (isEditing: boolean) => {
        if (!planName.trim()) { Alert.alert('خطأ', 'يرجى إدخال اسم الخطة'); return; }
        if (selectedExercises.length === 0) { Alert.alert('خطأ', 'يرجى اختيار تمرين واحد على الأقل'); return; }
        const planData = { name: planName.trim(), description: planDescription.trim() || 'خطة تدريب مخصصة', exercises: selectedExercises };
        if (isEditing && editingPlanId) { updateTrainingPlan(editingPlanId, planData); }
        else { addTrainingPlan({ ...planData, active: false }); }
        setShowAddModal(false); setShowEditModal(false); setEditingPlanId(null);
    };

    const toggleExerciseSelection = (exerciseId: number) => setSelectedExercises(prev => prev.includes(exerciseId) ? prev.filter(id => id !== exerciseId) : [...prev, exerciseId]);

    return (
        <View style={styles.viewContainer}>
            <ScrollView style={styles.listScrollView} showsVerticalScrollIndicator={false}>
                {trainingPlans.length === 0 ? <View style={styles.emptyContainer}><Text style={styles.emptyText}>لا توجد خطط تدريب مضافة بعد</Text></View> : trainingPlans.map((plan) => (
                    <View key={plan.id} style={styles.planCard}>
                        <View style={styles.planHeader}><View style={styles.planTitleContainer}><Text style={styles.planName}>{plan.name}</Text>{activePlanId === plan.id && <View style={styles.activeBadge}><Text style={styles.activeBadgeText}>مفعلة</Text></View>}</View><View style={styles.planActions}><TouchableOpacity style={[styles.actionButton, { backgroundColor: activePlanId === plan.id ? '#4cc9f0' : settings.primaryColor }]} onPress={() => handleActivatePlan(plan.id)}>{activePlanId === plan.id ? <CheckCircle size={18} color="white" /> : <Play size={18} color="white" />}</TouchableOpacity><TouchableOpacity style={[styles.actionButton, { backgroundColor: settings.primaryColor }]} onPress={() => handleEditPlan(plan)}><Edit size={18} color="white" /></TouchableOpacity><TouchableOpacity style={[styles.actionButton, styles.deleteButton]} onPress={() => handleDeletePlan(plan.id)}><Trash2 size={18} color="white" /></TouchableOpacity></View></View>
                        <Text style={styles.planDescription}>{plan.description}</Text>
                        <View style={styles.exercisesContainer}>{plan.exercises.map(id => exercises.find(e => e.id === id)?.name).filter(Boolean).map((name, index) => <View key={index} style={styles.planExerciseItem}><Text style={styles.planExerciseText}>{name}</Text></View>)}</View>
                    </View>
                ))}
                {activePlanId !== null && <TouchableOpacity style={[styles.deactivateButton, styles.dangerButton]} onPress={() => handleActivatePlan(activePlanId)}><XCircle size={20} color="white" /><Text style={styles.deactivateButtonText}>إلغاء تفعيل الخطة الحالية</Text></TouchableOpacity>}
            </ScrollView>
            <TouchableOpacity style={[styles.addButton, { backgroundColor: settings.primaryColor }]} onPress={handleAddNewPlan}><Plus size={20} color="white" /><Text style={styles.addButtonText}>إنشاء خطة تدريب جديدة</Text></TouchableOpacity>
            <Modal visible={showAddModal || showEditModal} transparent={true} animationType="slide" onRequestClose={() => {setShowAddModal(false); setShowEditModal(false);}}><View style={styles.modalOverlay}><View style={styles.modalContent}><View style={styles.modalHeader}><Text style={styles.modalTitle}>{showEditModal ? 'تعديل خطة التدريب' : 'إنشاء خطة تدريب جديدة'}</Text><TouchableOpacity onPress={() => {setShowAddModal(false); setShowEditModal(false);}}><X size={24} color="#6b7280" /></TouchableOpacity></View><View style={styles.inputContainer}><Text style={styles.inputLabel}>اسم الخطة</Text><TextInput style={styles.input} value={planName} onChangeText={setPlanName} placeholder="مثال: خطة المبتدئين" /></View><View style={styles.inputContainer}><Text style={styles.inputLabel}>الوصف</Text><TextInput style={[styles.input, styles.textArea]} value={planDescription} onChangeText={setPlanDescription} placeholder="وصف الخطة..." multiline numberOfLines={3} /></View><View style={styles.inputContainer}><Text style={styles.inputLabel}>اختر التمارين ({selectedExercises.length})</Text><ScrollView style={styles.exercisesList}>{exercises.map(exercise => (<TouchableOpacity key={exercise.id} style={[styles.exerciseSelectItem, selectedExercises.includes(exercise.id) && styles.exerciseSelectItemSelected]} onPress={() => toggleExerciseSelection(exercise.id)}><View style={styles.exerciseSelectInfo}><Text style={styles.exerciseSelectName}>{exercise.name}</Text><Text style={styles.exerciseSelectMuscle}>{exercise.muscle}</Text></View>{selectedExercises.includes(exercise.id) && <CheckCircle size={20} color={settings.primaryColor} />}</TouchableOpacity>))}</ScrollView></View><View style={styles.modalButtons}><TouchableOpacity style={[styles.modalButton, styles.cancelButton]} onPress={() => {setShowAddModal(false); setShowEditModal(false);}}><Text style={styles.cancelButtonText}>إلغاء</Text></TouchableOpacity><TouchableOpacity style={[styles.modalButton, { backgroundColor: settings.primaryColor }]} onPress={() => handleSavePlan(!!showEditModal)}><Text style={styles.confirmButtonText}>{showEditModal ? 'حفظ' : 'إنشاء'}</Text></TouchableOpacity></View></View></View></Modal>
        </View>
    );
};
...
const styles = StyleSheet.create({
    ...
    switchButton: { flex: 1, paddingVertical: 10, borderRadius: 8, alignItems: 'center' },
    switchButtonText: { fontSize: 14, fontWeight: '600', color: '#374151' },
    activeSwitchButtonText: { color: 'white' },
    contentScrollView: { flex: 1, paddingHorizontal: 16, paddingBottom: 20 },
    viewContainer: { flex: 1 },
    listScrollView: { flex: 1 },
    controlsContainer: { flexDirection: 'row', paddingBottom: 15, gap: 10 },
    controlButton: { flex: 1, paddingVertical: 8, paddingHorizontal: 15, borderRadius: 8, alignItems: 'center' },
    secondaryButton: { backgroundColor: '#6c757d' },
    dangerButton: { backgroundColor: '#e63946' },
    controlButtonText: { color: 'white', fontSize: 12, fontWeight: 'bold' },
    ...
    selectedExerciseItem: { backgroundColor: '#e6f7ff' },
    exerciseContent: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
    exerciseInfo: { flex: 1 },
    exerciseName: { fontSize: 16, fontWeight: 'bold', color: '#212529', marginBottom: 4 },
    exerciseDetails: { fontSize: 14, color: '#6c757d', marginBottom: 4 },
    exerciseStatus: { fontSize: 12, fontWeight: 'bold' },
    exerciseActions: { flexDirection: 'row', gap: 10 },
    ...
    addButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginVertical: 20, paddingVertical: 15, borderRadius: 10, gap: 8 },
    addButtonText: { color: 'white', fontSize: 16, fontWeight: 'bold' },
    modalOverlay: { flex: 1, backgroundColor: 'rgba(0, 0, 0, 0.5)', justifyContent: 'center', alignItems: 'center', padding: 20 },
    modalContent: { backgroundColor: 'white', borderRadius: 16, padding: 20, width: '100%', maxWidth: 500, maxHeight: '90%' },
    libraryModalContent: { backgroundColor: 'white', borderRadius: 16, padding: 20, width: '100%', maxWidth: 500, maxHeight: '90%' },
    ...
    inputFlex: { flex: 1, borderWidth: 1, borderColor: '#d1d5db', borderRadius: 8, padding: 12, fontSize: 16, backgroundColor: '#f9fafb' },
    libraryButton: { width: 48, height: 48, borderRadius: 8, justifyContent: 'center', alignItems: 'center' },
    muscleSelector: { maxHeight: 50 },
    muscleButton: { backgroundColor: '#f3f4f6', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, marginRight: 8, borderWidth: 1, borderColor: '#d1d5db' },
    selectedMuscleButton: { backgroundColor: '#3b82f6', borderColor: '#3b82f6' },
    muscleButtonText: { fontSize: 14, fontWeight: '600', color: '#6b7280' },
    selectedMuscleButtonText: { color: 'white' },
    ...
    modalMessage: { fontSize: 16, color: '#6b7280', textAlign: 'center', marginBottom: 20, lineHeight: 24 },
    searchContainer: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f9fafb', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 16, borderWidth: 1, borderColor: '#e5e7eb', gap: 8 },
    searchInput: { flex: 1, fontSize: 14, color: '#111827' },
    muscleTabsContainer: { marginBottom: 16, maxHeight: 50 },
    muscleTab: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, marginRight: 8, backgroundColor: '#f3f4f6', borderWidth: 1, borderColor: '#d1d5db' },
    selectedMuscleTab: { backgroundColor: '#3b82f6', borderColor: '#3b82f6' },
    muscleTabText: { fontSize: 13, fontWeight: '600', color: '#6b7280' },
    selectedMuscleTabText: { color: 'white' },
    libraryScrollView: { flex: 1 },
    libraryExerciseItem: { flexDirection: 'row', backgroundColor: '#f9fafb', borderRadius: 12, padding: 12, marginBottom: 12, borderWidth: 1, borderColor: '#e5e7eb', gap: 12 },
    libraryExerciseImagePlaceholder: { width: 60, height: 60, backgroundColor: '#e5e7eb', borderRadius: 8, justifyContent: 'center', alignItems: 'center' },
    libraryExerciseInfo: { flex: 1 },
    libraryExerciseName: { fontSize: 15, fontWeight: '700', color: '#111827', marginBottom: 2 },
    libraryExerciseNameEn: { fontSize: 12, color: '#6b7280', marginBottom: 4 },
    libraryExerciseDescription: { fontSize: 12, color: '#6b7280', marginBottom: 6, lineHeight: 16 },
    libraryExerciseMeta: { flexDirection: 'row', gap: 12 },
    libraryMetaItem: { flexDirection: 'row', alignItems: 'center', gap: 4 },
    libraryMetaLabel: { fontSize: 10, color: '#9ca3af', fontWeight: '600' },
    libraryMetaValue: { fontSize: 10, color: '#4b5563', fontWeight: '600' },
    tabContainer: { flexDirection: 'row', paddingBottom: 15, gap: 8 },
    tab: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 10, paddingHorizontal: 12, borderRadius: 8, backgroundColor: 'white', borderWidth: 1, borderColor: '#e2e8f0', gap: 6 },
    ...
    calendar: { borderWidth: 1, borderColor: '#e2e8f0', borderRadius: 12 },
    sessionCard: { backgroundColor: 'white', borderRadius: 12, padding: 16, marginBottom: 12, borderLeftWidth: 4, borderLeftColor: '#4ade80' },
    sessionPlanName: { fontSize: 16, fontWeight: '600', color: '#1f2937' },
    sessionTime: { fontSize: 13, color: '#6b7280', marginTop: 4 },
    ...
    planTitleContainer: { flexDirection: 'row', alignItems: 'center', flex: 1 },
    planName: { fontSize: 18, fontWeight: 'bold', color: '#212529' },
    activeBadge: { backgroundColor: '#4cc9f0', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 12, marginLeft: 10 },
    activeBadgeText: { color: 'white', fontSize: 10, fontWeight: 'bold' },
    planActions: { flexDirection: 'row', gap: 10 },
    planDescription: { fontSize: 14, color: '#6c757d', marginBottom: 15, lineHeight: 20 },
    exercisesContainer: { marginTop: 10 },
    planExerciseItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: '#eee' },
    planExerciseText: { fontSize: 14, color: '#212529' },
    deactivateButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 15, borderRadius: 10, marginTop: 20, gap: 8 },
    deactivateButtonText: { color: 'white', fontSize: 16, fontWeight: 'bold' },
    textArea: { minHeight: 80, textAlignVertical: 'top' },
    exercisesList: { maxHeight: 200, borderWidth: 1, borderColor: '#e5e7eb', borderRadius: 8, backgroundColor: '#f9fafb' },
    ...
    exerciseSelectItemSelected: { backgroundColor: '#eff6ff' },
    exerciseSelectInfo: { flex: 1 },
    exerciseSelectName: { fontSize: 14, fontWeight: '600', color: '#111827', marginBottom: 2 },
